function [D,Gamma,err,gerr] = box_ksvd_scale_hf(params,varargin)
...
for iter = 1:iternum
  ...
  fprintf('\nDownscaled dictionary energy = %f\n', norm(W*D, 'fro'));
  if (msgdelta>0)
...
function Gamma = constrained_sparsecode(data, D, X1, l0_resolve, thresh)
    ...
    row_idx = zeros(thresh, size(data, 2));
    col_idx = zeros(thresh, size(data, 2));
    values = zeros(thresh, size(data, 2));
	...
	for idx = 1:1:size(data, 2)
        ...
        nz_idx = find(X0_sub ~= 0);
        if length(nz_idx) == 0
            continue;
        end
        if length(nz_idx) == thresh
            row_idx(:, idx) = repeated_cset(X0_sub ~= 0);
            col_idx(:, idx) = idx*ones(thresh, 1);
            values(:, idx) = X0_sub(X0_sub ~= 0);
        else
            r = thresh - length(nz_idx);
            row_res = repeated_cset(nz_idx(end))*ones(r, 1);
            val_res = X0_sub(nz_idx(end))*ones(r, 1);
            row_idx(:, idx) = [repeated_cset(X0_sub ~= 0)'; row_res];
            values(:, idx) = [X0_sub(X0_sub ~= 0); val_res];
            col_idx(:, idx) = idx*ones(thresh, 1);
        end
	end
	% Save the matrix as sparse matrix.
    nnz_idx = find(row_idx ~= 0);
    Gamma = sparse(row_idx(nnz_idx), col_idx(nnz_idx), values(nnz_idx), size(D, 2), size(data, 2));
end
